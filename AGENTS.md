...
- **Source columns** (in `nemar-db.datasets`, all indexed): `visibility`, `status`, `concept_doi`, `zarr_status`, `archive_status`, `archive_skip_reason`, `license_tier`, `modalities`, `file_size`, `last_activity_at`. Archive + access instrumentation lives in nemar-cli (epic #695 / issue #696).
